...
public:
  ...
  const char* aggr_func() const{ 
    static const char *FUNC_TYPE_NAME[] = {"max","min","count","avg","sum"};
    return FUNC_TYPE_NAME[aggr_type_]; 
  };

  const char* aggr_name() const{ 
    return aggr_name_; 
  };
